...
func registerQueryRoutes(cliCtx client.Context, r *mux.Router) {
	...
	r.HandleFunc(fmt.Sprintf("/%s/circulatingsupplyfusd", types.QueryPath), getCirculatingSupplyFUSDHandlerFn(cliCtx)).Methods("GET")
	...
	r.HandleFunc(fmt.Sprintf("/%s/totalsupplyfusd", types.QueryPath), getTotalSupplyFUSDHandlerFn(cliCtx)).Methods("GET")
}
...
func getCirculatingSupplyFUSDHandlerFn(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		_, page, limit, err := rest.ParseHTTPArgsWithLimit(r, 0)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
		...
		route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryCirculatingSupplyFUSD)
		res, _, err := cliCtx.QueryWithData(route, bz)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
...
func getTotalSupplyFUSDHandlerFn(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		_, page, limit, err := rest.ParseHTTPArgsWithLimit(r, 0)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
		...
		route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryTotalSupplyFUSD)
		res, _, err := cliCtx.QueryWithData(route, bz)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
