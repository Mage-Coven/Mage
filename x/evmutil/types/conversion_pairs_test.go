...
func TestConversionPair_GetAddress(t *testing.T) {
	...
	require.Equal(t, types.HexBytes(addr.Bytes()), pair.MageERC20Address, "struct address should match input bytes")
...
func TestConversionPairs_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			errArgs{
				expectPass: false,
				contains:   "found duplicate enabled conversion pair denom mage",
			},
		},
		{
