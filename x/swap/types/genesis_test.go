    ...
    "params": {
		...
		"pool_records": [
			...
			},
		  {
			  "pool_id": "hard:fusd",
			  "reserves_a": { "denom": "umage", "amount": "1000000" },
			  "reserves_b": { "denom": "fusd", "amount": "2000000" },
...
`
	...
	state := types.NewGenesisState(
		...
		types.PoolRecords{
			types.NewPoolRecord(sdk.NewCoins(umage(1e6), fusd(5e6)), i(3e6)),
			types.NewPoolRecord(sdk.NewCoins(hard(1e6), fusd(2e6)), i(15e5)),
		},
		types.ShareRecords{
			types.NewShareRecord(depositor_1, types.PoolID("umage", "fusd"), i(1e5)),
			types.NewShareRecord(depositor_2, types.PoolID("hard", "fusd"), i(2e5)),
		},
	)
...
func TestGenesis_ValidatePoolRecords(t *testing.T) {
	invalidPoolRecord := types.NewPoolRecord(sdk.NewCoins(umage(1e6), fusd(5e6)), i(-1))
...
func TestGenesis_Validate_PoolShareIntegration(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			shareRecords: types.ShareRecords{},
			expectedErr:  "total depositor shares 0 not equal to pool 'umage:fusd' total shares 3000000",
		},
		{
			name:        "zero pool records, one share record",
			poolRecords: types.PoolRecords{},
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("umage", "fusd"), i(5e6)),
			},
			expectedErr: "total depositor shares 5000000 not equal to pool 'umage:fusd' total shares 0",
		},
		{
			...
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, "umage:fusd", i(15e5)),
			},
			expectedErr: "total depositor shares 1500000 not equal to pool 'umage:fusd' total shares 3000000",
		},
		{
			name: "more than one pool records, more than one share record",
			poolRecords: types.PoolRecords{
				types.NewPoolRecord(sdk.NewCoins(umage(1e6), fusd(5e6)), i(3e6)),
				types.NewPoolRecord(sdk.NewCoins(hard(1e6), fusd(2e6)), i(2e6)),
			},
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("umage", "fusd"), i(15e5)),
				types.NewShareRecord(depositor_2, types.PoolID("umage", "fusd"), i(15e5)),
				types.NewShareRecord(depositor_1, types.PoolID("hard", "fusd"), i(1e6)),
			},
			expectedErr: "total depositor shares 1000000 not equal to pool 'hard:fusd' total shares 2000000",
		},
		{
			...
			poolRecords: types.PoolRecords{
				types.NewPoolRecord(sdk.NewCoins(umage(1e6), fusd(5e6)), i(3e6)),
				types.NewPoolRecord(sdk.NewCoins(hard(1e6), fusd(2e6)), i(2e6)),
				types.NewPoolRecord(sdk.NewCoins(hard(7e6), umage(10e6)), i(8e6)),
			},
			shareRecords: types.ShareRecords{
				types.NewShareRecord(depositor_1, types.PoolID("umage", "fusd"), i(15e5)),
				types.NewShareRecord(depositor_2, types.PoolID("umage", "fusd"), i(15e5)),
				types.NewShareRecord(depositor_1, types.PoolID("hard", "fusd"), i(2e6)),
				types.NewShareRecord(depositor_1, types.PoolID("hard", "umage"), i(3e6)),
				types.NewShareRecord(depositor_2, types.PoolID("hard", "umage"), i(5e6)),
			},
			expectedErr: "",
		},
	}
