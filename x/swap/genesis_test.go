...
func (suite *genesisTestSuite) Test_InitAndExportGenesis() {
	...
	state := types.NewGenesisState(
		types.Params{
			AllowedPools: types.AllowedPools{types.NewAllowedPool("umage", "fusd")},
			SwapFee:      sdk.MustNewDecFromStr("0.00255"),
		},
		types.PoolRecords{
			types.NewPoolRecord(sdk.NewCoins(sdk.NewCoin("hard", sdk.NewInt(1e6)), sdk.NewCoin("fusd", sdk.NewInt(2e6))), sdk.NewInt(1e6)),
			types.NewPoolRecord(sdk.NewCoins(sdk.NewCoin("umage", sdk.NewInt(1e6)), sdk.NewCoin("fusd", sdk.NewInt(5e6))), sdk.NewInt(3e6)),
		},
		types.ShareRecords{
			types.NewShareRecord(depositor_2, types.PoolID("hard", "fusd"), sdk.NewInt(1e6)),
			types.NewShareRecord(depositor_1, types.PoolID("umage", "fusd"), sdk.NewInt(3e6)),
		},
	)
	...
	poolRecord1, _ := suite.Keeper.GetPool(suite.Ctx, types.PoolID("hard", "fusd"))
	suite.Equal(state.PoolRecords[0], poolRecord1)
	poolRecord2, _ := suite.Keeper.GetPool(suite.Ctx, types.PoolID("umage", "fusd"))
	suite.Equal(state.PoolRecords[1], poolRecord2)

	shareRecord1, _ := suite.Keeper.GetDepositorShares(suite.Ctx, depositor_2, types.PoolID("hard", "fusd"))
	suite.Equal(state.ShareRecords[0], shareRecord1)
	shareRecord2, _ := suite.Keeper.GetDepositorShares(suite.Ctx, depositor_1, types.PoolID("umage", "fusd"))
