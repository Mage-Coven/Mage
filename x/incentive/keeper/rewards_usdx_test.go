...
type FUSDIntegrationTests struct {
...
func TestFUSDIntegration(t *testing.T) {
	suite.Run(t, new(FUSDIntegrationTests))
}

// SetupTest is run automatically before each suite test
func (suite *FUSDIntegrationTests) SetupTest() {
	_, suite.addrs = app.GeneratePrivKeyAddressPairs(5)

	suite.genesisTime = time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC)
}

func (suite *FUSDIntegrationTests) ProposeAndVoteOnNewRewardPeriods(committeeID uint64, voter sdk.AccAddress, newPeriods types.RewardPeriods) {
	suite.ProposeAndVoteOnNewParams(
		voter,
		committeeID,
		[]proposaltypes.ParamChange{{
			Subspace: types.ModuleName,
			Key:      string(types.KeyFUSDMintingRewardPeriods),
			Value:    string(types.ModuleCdc.LegacyAmino.MustMarshalJSON(newPeriods)),
		}})
}

func (suite *FUSDIntegrationTests) TestSingleUserAccumulatesRewardsAfterSyncing() {
	userA := suite.addrs[0]

	authBulder := app.NewAuthBankGenesisBuilder().
		WithSimpleModuleAccount(magedisttypes.ModuleName, cs(c(types.FUSDMintingRewardDenom, 1e18))). // Fill magedist with enough coins to pay out any reward
	...
	incentBuilder := testutil.NewIncentiveGenesisBuilder().
		...
		WithMultipliers(types.MultipliersPerDenoms{{
			Denom:       types.FUSDMintingRewardDenom,
	...
	msg := types.NewMsgClaimFUSDMintingReward(userA.String(), "large")
	...
	suite.BalanceInEpsilon(userA, cs(c("bnb", 1e12-1e10), c(cdptypes.DefaultStableDenom, 1e9), c(types.FUSDMintingRewardDenom, 2*1e6*1e6)), accuracy)
}

func (suite *FUSDIntegrationTests) TestSingleUserAccumulatesRewardsWithoutSyncing() {
	user := suite.addrs[0]
	...
	incentBuilder := testutil.NewIncentiveGenesisBuilder().
		...
		WithSimpleFUSDRewardPeriod(collateralType, c(types.FUSDMintingRewardDenom, 1e6))
	...
	suite.NoError(
		suite.DeliverMsgCreateCDP(user, initialCollateral, c("fusd", 1e8), collateralType),
	)

	// Skip ahead a few blocks blocks to accumulate both interest and fusd reward for the cdp
	// Don't sync the CDP between the blocks
	suite.NextBlockAfter(1e6 * time.Second) // about 12 days
	suite.NextBlockAfter(1e6 * time.Second)
	suite.NextBlockAfter(1e6 * time.Second)

	msg := types.NewMsgClaimFUSDMintingReward(user.String(), "large")
	suite.NoError(suite.DeliverIncentiveMsg(&msg))

	// The users has always had 100% of cdp debt, so they should receive all rewards for the previous two blocks.
	// Total rewards for each block is block duration * rewards per second
	accuracy := 1e-18 // using a very high accuracy to flag future small calculation changes
	suite.BalanceInEpsilon(user, cs(c(cdptypes.DefaultStableDenom, 1e8), c(types.FUSDMintingRewardDenom, 3*1e6*1e6)), accuracy)
}

func (suite *FUSDIntegrationTests) TestReinstatingRewardParamsDoesNotTriggerOverPayments() {
	userA := suite.addrs[0]
	userB := suite.addrs[1]
	...
	suite.NoError(
		suite.DeliverMsgCreateCDP(userA, c("bnb", 1e10), c("fusd", 1e9), "bnb-a"),
	)
	suite.NextBlockAfter(1e6 * time.Second)

	// Remove the FUSD reward period
	...
	// Withdraw the same amount of fusd as the first cdp currently has. This make the reward maths easier, as rewards will be split 50:50 between each cdp.
	...
	suite.ProposeAndVoteOnNewRewardPeriods(0, userA,
		types.RewardPeriods{types.NewRewardPeriod(
			...
			c(types.FUSDMintingRewardDenom, 1e6),
		)},
	)
	// next block so proposal is enacted
	suite.NextBlockAfter(1 * time.Second)
...
// SetupTest is run automatically before each suite test
func (suite *FUSDRewardsTestSuite) SetupTest() {
...
func (suite *FUSDRewardsTestSuite) SetupApp() {
...
func (suite *FUSDRewardsTestSuite) SetupWithGenState(authBuilder *app.AuthBankGenesisBuilder, incentBuilder testutil.IncentiveGenesisBuilder) {
	suite.SetupApp()
...
func (suite *FUSDRewardsTestSuite) TestAccumulateFUSDMintingRewards() {
	type args struct {
		ctype                 string
	...
	testCases := []test{
		{
			...
			args{
				ctype:                 "bnb-a",
				rewardsPerSecond:      c("umage", 122354),
				initialTotalPrincipal: c("fusd", 1000000000000),
				timeElapsed:           7,
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			incentBuilder := testutil.NewIncentiveGenesisBuilder().WithGenesisTime(suite.genesisTime).WithSimpleFUSDRewardPeriod(tc.args.ctype, tc.args.rewardsPerSecond)
			...
			rewardPeriod, found := suite.keeper.GetFUSDMintingRewardPeriod(suite.ctx, tc.args.ctype)
			suite.Require().True(found)
			suite.keeper.AccumulateFUSDMintingRewards(suite.ctx, rewardPeriod)

			rewardFactor, _ := suite.keeper.GetFUSDMintingRewardFactor(suite.ctx, tc.args.ctype)
			suite.Require().Equal(tc.args.expectedRewardFactor, rewardFactor)
		})
	}
}

func (suite *FUSDRewardsTestSuite) TestSynchronizeFUSDMintingReward() {
	type args struct {
		ctype                string
		rewardsPerSecond     sdk.Coin
	...
	testCases := []test{
		{
			...
			args{
				...
				initialPrincipal:     c("fusd", 10000000000),
				blockTimes:           []int{10, 10, 10, 10, 10, 10, 10, 10, 10, 10},
				expectedRewardFactor: d("0.001223540000000000"),
				expectedRewards:      c("umage", 12235400),
			},
		},
		{
			...
			args{
				...
				expectedRewards:      c("umage", 105713856000),
			},
		},
	}
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			claim, found := suite.keeper.GetFUSDMintingClaim(suite.ctx, suite.addrs[0])
			suite.Require().True(found)
			...
			for _, t := range tc.args.blockTimes {
				...
				rewardPeriod, found := suite.keeper.GetFUSDMintingRewardPeriod(blockCtx, tc.args.ctype)
				suite.Require().True(found)
				suite.keeper.AccumulateFUSDMintingRewards(blockCtx, rewardPeriod)
			}
			...
			suite.Require().NotPanics(func() {
				suite.keeper.SynchronizeFUSDMintingReward(suite.ctx, cdp)
			})
...
func (suite *FUSDRewardsTestSuite) TestSimulateFUSDMintingRewardSynchronization() {
	type args struct {
		ctype                string
		rewardsPerSecond     sdk.Coin
		initialCollateral    sdk.Coin
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().Equal(claim.Reward, sdk.NewCoin("umage", sdk.ZeroInt()))

			updatedClaim := suite.keeper.SimulateFUSDMintingSynchronization(suite.ctx, claim)
...
func TestFUSDRewardsTestSuite(t *testing.T) {
	suite.Run(t, new(FUSDRewardsTestSuite))
}
